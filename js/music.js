...
function buildUI() {
  ...
  uploadInput.accept = 'audio/*,.mp3,.wav,.ogg,.m4a,.aac,.flac';
  uploadInput.multiple = true;
  Object.assign(uploadInput.style, { position: 'absolute', left: '-9999px' });
  document.body.appendChild(uploadInput);

  // --- glass controls container (matches dock pill style) ---
  ...
  // --- scrub bar (div-based, clean) ---
  const scrubOuter = mk('div', { position: 'relative', width: '100%', height: '2px', cursor: 'pointer' });
  scrubOuter.appendChild(mk('div', {
    position: 'absolute', top: '0', left: '0', width: '100%', height: '100%',
    background: 'rgba(230,232,240,0.18)', borderRadius: '999px',
  }));
  const scrubFill = mk('div', {
    position: 'absolute', top: '0', left: '0', width: '0%', height: '100%',
    background: 'rgba(230,232,240,0.6)', borderRadius: '999px', pointerEvents: 'none',
  });
  scrubOuter.appendChild(scrubFill);
  const prog = document.createElement('input');
  prog.type = 'range'; prog.min = '0'; prog.max = '1'; prog.step = '0.001'; prog.value = '0';
  Object.assign(prog.style, {
    position: 'absolute', top: '-8px', left: '0',
    width: '100%', height: '18px',
    opacity: '0', cursor: 'pointer', margin: '0', padding: '0',
  });
  scrubOuter.appendChild(prog);
  const setScrubFill = () => { scrubFill.style.width = (Number(prog.value) * 100).toFixed(2) + '%'; };
  prog.addEventListener('input', setScrubFill);

  // --- icon buttons (Material Symbols, circle style) ---
  const dotBtn = (title, iconName) => {
    ...
    b.addEventListener('mouseenter', () => { b.style.borderColor = 'rgba(230,232,240,0.8)'; b.style.background = 'rgba(230,232,240,0.08)'; });
    b.addEventListener('mouseleave', () => { b.style.borderColor = 'rgba(230,232,240,0.35)'; b.style.background = 'transparent'; });
  ...
  const vinylBtn  = dotBtn('DJ scrub disc', 'album');
  const uploadBtn = dotBtn('Upload audio', 'upload');
  ...
  row.appendChild(vinylBtn);
  row.appendChild(uploadBtn);

  ctrlWrap.appendChild(scrubOuter);
  ctrlWrap.appendChild(row);

  // assemble column
  column.appendChild(vizBox);
  column.appendChild(ctrlWrap);
  ...
  vinylBtn.addEventListener('click', () => window.__MIDDLE_UI?.toggleVinyl?.());
  uploadBtn.addEventListener('click', () => uploadInput.click());
  uploadInput.addEventListener('change', () => {
    const files = Array.from(uploadInput.files || []);
    if (files.length) window.__MUSIC_API?.addFiles?.(files);
    uploadInput.value = '';
  });
  ...
  prevBtn.addEventListener('click', () => {
    ...
    loadTrack(isPlaying, updateTitle);
    notifyPlaylist();
  });
  ...
  audio.addEventListener('timeupdate', () => {
    ...
    if (!prog.matches(':active')) { prog.value = String(audio.currentTime / audio.duration); setScrubFill(); }
  });
  ...
  function frame() {
    if (analyser && !audio.paused && !audio.ended) {
      ...
      // idle wave — gentle sine ripple across bars when no music is playing
      const t = performance.now() * 0.001;
      for (let i = 0; i < barsRef.length; i++) {
        const phase = (i / barsRef.length) * Math.PI * 4;
        const wave = 0.5 + 0.5 * Math.sin(t * 0.6 + phase);
        const h = Math.round(1 + wave * (SIZE.vizH * 0.12));
        barsRef[i].style.height = `${h}px`;
        barsRef[i].style.opacity = String(0.12 + wave * 0.10);
        capsRef[i].style.opacity = '0'; // hide caps during idle
      }
      publishAudioBus();
    }
...
function buildAPI(isStub){
  const api = {
    ...
    // Live stream injection (radio mode)
    setStream: async (url, title) => {
      await ensureAudioContext();
      // Inject / replace a 'stream' slot at front of playlist
      const track = { id: '__radio__', title: title || 'Live Radio', src: url, kind: 'stream' };
      const idx = playlist.findIndex(t => t.id === '__radio__');
      if (idx >= 0) playlist.splice(idx, 1, track); else playlist.unshift(track);
      trackIdx = 0;
      audio.src = url;
      audio.load();
      const go = () => audio.play().catch(() => {});
      ...
      _ui.updateTitle?.();
      emit('titlechange', { title: track.title });
    },
    ...
    stopStream: () => {
      ...
      trackIdx = Math.max(0, Math.min(trackIdx, playlist.length - 1));
      if (playlist.length) {
        loadTrack(false, _ui.updateTitle);
        emit('titlechange', { title: currentTrack()?.title || '' });
      } else {
        audio.pause();
        audio.removeAttribute('src');
        audio.load();
